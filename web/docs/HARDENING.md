...
- Transcription admissions price the declared bytes at the worst-case low bitrate before Whisper runs, then settle to the provider-reported duration, clamped so a request can never commit more than it reserved. Inference is aborted after four minutes, below the six-minute reservation expiry, so a live request always settles before its reservation can be reclaimed and a stalled upload cannot hold budget indefinitely.
- The default ceilings are `DAILY_SPEND_LIMIT_MICROS = 2000000` (2.00 USD per UTC day across all inference) and `DAILY_CLIENT_AUDIO_SECONDS = 7200` (two hours of transcribed audio per client IP per UTC day). Raising them is a deliberate `wrangler.jsonc` change, not a dashboard toggle.
