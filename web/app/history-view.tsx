  ...
  return (
    <section className="history-workspace">
      ...
      <div className="history-card history-library">
        ...
        {visible.length ? <div className="history-library-list">{visible.map((item) => <article className="history-library-item" key={item._id}><div className="history-item-meta"><span>{new Date(item.createdAt).toLocaleString()}</span><span>{item.durationSeconds}s · {item.model}{item.detectedLanguage ? ` · ${item.detectedLanguage}` : ""}</span></div><h2>Transcript</h2>{item.decryptError ? <p className="history-transcript empty">This item could not be decrypted and can only be deleted.</p> : <p className="history-transcript">{item.text}</p>}<h2>Summary</h2>{item.summary ? <p className="history-summary">{item.summary}</p> : <p className="history-summary empty">{item.summaryDecryptError ? "This summary could not be decrypted." : "No summary generated yet."}</p>}<div className="history-library-tools"><button onClick={() => props.onOpen(item)}>Open in studio</button><button onClick={() => props.onSummarize(item)}>{item.summary ? "Regenerate summary" : "Generate summary"}</button><button onClick={() => props.onNarrate(item.summary ?? item.text)}>Narrate {item.summary ? "summary" : "transcript"}</button><button onClick={() => downloadTranscript(item.text, "txt")}>TXT</button><button className="delete" onClick={() => props.onDelete(item._id)}>Delete</button></div></article>)}</div> : <p className="history-empty">{search ? "No transcripts or summaries match your search." : "Your completed recordings will appear here automatically."}</p>}
