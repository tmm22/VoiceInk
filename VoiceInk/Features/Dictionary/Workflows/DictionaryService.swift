...
enum DictionaryService {
    private static let logger = Logger(subsystem: AppLogger.subsystem, category: "DictionaryService")
    ...
    static func removeExactDuplicateContent(context: ModelContext, source: String) -> Bool {
        ...
        do {
            ...
            context.rollback()
            logger.error(
                "Failed to remove exact dictionary duplicates from \(source, privacy: .public): \(AppLogger.errorMetadata(error), privacy: .public)"
            )
            return false
        }
    }
