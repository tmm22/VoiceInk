...
class VoiceInkEngine: NSObject, ObservableObject {
    ...
    init(
        ...
        let appSupportDirectory = AppBrand.applicationSupportDirectory()
                ...
                if granted {
                    Task { @MainActor [self] in
                        ...
                        do {
                            ...
                            let pendingAudioChunks = PendingAudioChunkBuffer()
                            self.recorder.onAudioChunk = pendingAudioChunks.append
                            ...
                            if self.serviceRegistry.shouldUseRealtimeTranscription(for: transcriptionConfiguration) {
                                ...
                                if let realCallback {
                                    let droppedStartupChunks = pendingAudioChunks.handoff(to: realCallback)
                                    if droppedStartupChunks > 0 {
                                        self.logger.warning(
                                            "Realtime startup buffer overflowed by \(droppedStartupChunks, privacy: .public) chunks; using the complete disk recording"
                                        )
                                        session.requireBatchFallback()
                            ...
                            Task { @MainActor [weak self] in
                                    ...
                                    model.provider == .whisper
                                {
                                    if let localWhisperModel = self.whisperModelManager.availableModels.first(where: {
                                        ...
                                        self.whisperModelManager.loadedWhisperModel?.name != model.name
                                    {
                                        do {
                            ...
                            activeModeTask.cancel()
                            self.logger.error("Recording failed to start: \(AppLogger.errorMetadata(error), privacy: .public)")
    ...
    private func saveCanceledRecording() async {
        ...
        do {
            ...
            logger.error("Failed to save canceled recording: \(AppLogger.errorMetadata(error), privacy: .public)")
        }
    }
