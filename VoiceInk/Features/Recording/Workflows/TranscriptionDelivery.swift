...
final class TranscriptionDelivery {
    private let logger = Logger(subsystem: AppLogger.subsystem, category: "TranscriptionDelivery")
    ...
    private func runCustomCommand(command: String, commandText: String) async {
        ...
        do {
            ...
            if !result.stdout.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                logger.notice(
                    "Custom command produced stdout bytes=\(stdoutBytes, privacy: .public)"
                )
            }

            if !result.stderr.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                logger.notice(
                    "Custom command succeeded with stderr duration=\(Self.formattedDuration(duration), privacy: .public)s stdoutBytes=\(stdoutBytes, privacy: .public) stderrBytes=\(stderrBytes, privacy: .public)"
                )
            } else {
                logger.notice(
    ...
    private func notifyCustomCommandFailure(_ error: Error, duration: TimeInterval? = nil) {
        ...
        if let duration {
            logger.error(
                "Custom command failed duration=\(Self.formattedDuration(duration), privacy: .public)s type=\(errorType, privacy: .public)"
            )
        } else {
            logger.error("Custom command failed type=\(errorType, privacy: .public)")
        }
    }
    ...
    private func paste(_ text: String, output: OutputRuntimeConfiguration, actions: Actions) async {
        ...
        Task { @MainActor in
            CursorPaster.notifyIfSkipped(await pasteTask.value)
            await CursorPaster.autoSendAfterPaste(pasteTask, key: autoSendKey)
        }
    }
