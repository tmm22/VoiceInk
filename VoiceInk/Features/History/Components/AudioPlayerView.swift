    ...
    static func generateWaveformSamples(from url: URL, sampleCount: Int = 200) async -> [Float] {
        ...
        do {
            ...
            AppLogger.audio.error(
                "Failed to generate waveform: \(AppLogger.errorMetadata(error), privacy: .public)"
            )
            return []
        }
    }
...
class AudioPlayerManager: ObservableObject {
    ...
    func loadAudio(from url: URL) {
        do {
            ...
            AppLogger.audio.error(
                "Failed to load audio for playback: \(AppLogger.errorMetadata(error), privacy: .public)"
            )
        }
    }
