...
struct EnhancementSettingsView: View {
                ...
                VStack(spacing: 24) {
                    ...
                    if let aiService = enhancementService.getAIService() {
                        VStack(alignment: .leading, spacing: 16) {
                            ...
                            providerPicker(for: aiService)
                            modelPicker(for: aiService)
                            providerStatus(for: aiService)
                        }
                        .padding()
                        .background(CardBackground(isSelected: false))
                    }
    ...
    private func providerPicker(for aiService: AIService) -> some View {
        let providers = aiService.availableProviders
        let selection = Binding<AIProvider>(
            get: { aiService.selectedProvider },
            set: { newValue in
                aiService.selectedProvider = newValue
                if let first = aiService.availableModels.first {
                    aiService.selectModel(first)
                }
            }
        )
    ...
    private func modelPicker(for aiService: AIService) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("Model")
                .font(.subheadline)
                .foregroundColor(.secondary)

            if aiService.availableModels.count <= 1 {
    ...
    private func providerStatus(for aiService: AIService) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            switch aiService.selectedProvider {
            case .builtIn:
                Label("Runs entirely on-device. No configuration needed.", systemImage: "lock.shield")
                    .font(.caption)
                    .foregroundColor(.secondary)
            case .ollama:
                HStack(spacing: 12) {
                    if aiService.isReady {
                        Label("Ollama connected at \(ollamaBaseURL())", systemImage: "dot.radiowaves.left.and.right")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    } else {
                        Label("Start Ollama to enable local LLM post-processing.", systemImage: "exclamationmark.triangle")
                            .font(.caption)
    ...
    private func ollamaBaseURL() -> String {
        UserDefaults.standard.string(forKey: "ollamaBaseURL") ?? OllamaService.defaultBaseURL
    }
}
