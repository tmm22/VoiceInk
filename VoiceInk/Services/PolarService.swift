...
final class PolarService: PolarServicing {
    ...
    func checkLicenseRequiresActivation(_ key: String) async throws -> (
        ...
        if let httpResponse = httpResponse as? HTTPURLResponse {
            if !(200...299).contains(httpResponse.statusCode) {
                logger.error(
                    "License validation failed status=\(httpResponse.statusCode, privacy: .public) responseBytes=\(data.count, privacy: .public)"
                )
    ...
    func activateLicenseKey(_ key: String) async throws -> (activationId: String, activationsLimit: Int) {
        ...
        if let httpResponse = httpResponse as? HTTPURLResponse {
            if !(200...299).contains(httpResponse.statusCode) {
                logger.error(
                    "License activation failed status=\(httpResponse.statusCode, privacy: .public) responseBytes=\(data.count, privacy: .public)"
                )
                switch httpResponse.statusCode {
                case 404: throw LicenseError.keyNotFound
    ...
    func deactivateLicenseKey(_ key: String, activationId: String) async throws {
        ...
        guard httpResponse.statusCode == 204 else {
            ...
            logger.error(
                "License deactivation failed status=\(statusCode, privacy: .public) responseBytes=\(data.count, privacy: .public)"
            )
    ...
    func validateLicenseKeyWithActivation(_ key: String, activationId: String) async throws -> Bool {
        ...
        if let httpResponse = httpResponse as? HTTPURLResponse {
            if !(200...299).contains(httpResponse.statusCode) {
                logger.error(
                    "License validation with activation failed status=\(httpResponse.statusCode, privacy: .public) responseBytes=\(data.count, privacy: .public)"
                )
                switch httpResponse.statusCode {
                case 404: throw LicenseError.keyNotFound
                default: throw LicenseError.serverError(httpResponse.statusCode)
