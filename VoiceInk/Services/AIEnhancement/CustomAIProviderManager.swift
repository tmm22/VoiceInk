...
final class CustomAIProviderManager: ObservableObject {
    ...
    func requestConfiguration(forModel modelName: String) -> (baseURL: String, apiKey: String, modelName: String)? {
        guard let provider = provider(forModel: modelName),
            SecureEndpointValidator.isAllowed(provider.baseURL),
    ...
    func validateProvider(name: String, baseURL: String, model: String, excluding id: UUID? = nil) -> [String] {
        ...
        } else if !SecureEndpointValidator.isAllowed(trimmedURL) {
            errors.append(
                String(localized: "Base URL must use HTTPS (plain HTTP is allowed only for localhost)"))
        }
    ...
    private func loadProviders() {
        ...
        do {
            ...
            logger.error("Failed to decode custom AI providers: \(AppLogger.errorMetadata(error), privacy: .public)")
            providers = []
        }
    }
    ...
    private func saveProviders() {
        do {
            ...
            logger.error("Failed to encode custom AI providers: \(AppLogger.errorMetadata(error), privacy: .public)")
        }
    }
