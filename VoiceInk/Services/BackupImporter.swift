...
enum BackupImporter {
    ...
    static func apply(
        ...
        if categories.contains(.prompts) {
            ...
            AppLogger.storage.info("Imported prompts count=\(backup.customPrompts.count, privacy: .public)")
        }
        ...
        if categories.contains(.modes) {
            ...
            AppLogger.storage.info("Imported modes count=\(backup.modeConfigs.count, privacy: .public)")
        }
    ...
    private static func importGeneral(
        ...
        guard let general else {
            AppLogger.storage.info("Backup contained no general settings")
            return
        }
        ...
        AppLogger.storage.info("Imported general settings")
    }
    ...
    private static func importDictionary(from backup: BackupFile, modelContext: ModelContext) throws {
        ...
        if let words = backup.vocabularyWords {
            ...
            AppLogger.storage.info("Backup contained no vocabulary words")
        }
        ...
        if let replacements = backup.wordReplacements {
            ...
            AppLogger.storage.info("Backup contained no word replacements")
        }

        guard insertedWords > 0 || insertedReplacements > 0 else {
            AppLogger.storage.info("Backup produced no new dictionary entries")
            if skippedInvalidReplacements > 0 {
                AppLogger.storage.warning(
                    "Skipped invalid imported replacements count=\(skippedInvalidReplacements, privacy: .public)"
                )
            }
        ...
        do {
            ...
            AppLogger.storage.info(
                "Imported dictionary words=\(insertedWords, privacy: .public) replacements=\(insertedReplacements, privacy: .public)"
            )
            if skippedInvalidReplacements > 0 {
                AppLogger.storage.warning(
                    "Skipped invalid imported replacements count=\(skippedInvalidReplacements, privacy: .public)"
    ...
    private static func importCustomModels(
        ...
        guard let models else {
            AppLogger.storage.info("Backup contained no custom model definitions")
            return
        }
        ...
        AppLogger.storage.info("Imported custom model definitions count=\(models.count, privacy: .public)")
    }
