...
final class DashboardStatsSnapshotStore: @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: AppLogger.subsystem, category: "DashboardStatsSnapshotStore")
    ...
    private init(fileManager: FileManager = .default, userDefaults: UserDefaults = .standard) {
        ...
        let appSupportURL = AppBrand.applicationSupportDirectory(using: fileManager)
    ...
    func loadSnapshot() -> CachedSnapshot? {
        ...
        do {
            ...
            logger.error("Failed to load dashboard stats snapshot: \(AppLogger.errorMetadata(error), privacy: .public)")
            return nil
        }
    }
    ...
    func saveSummary(_ summary: DashboardStatsSummary) -> Metadata {
        ...
        saveQueue.async {
            do {
                ...
                logger.error("Failed to save dashboard stats snapshot: \(AppLogger.errorMetadata(error), privacy: .public)")
            }
        }
