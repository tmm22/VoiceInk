...
class NativeAppleTranscriptionService: TranscriptionService {
    private let logger = Logger(subsystem: AppLogger.subsystem, category: "NativeAppleTranscriptionService")
    ...
    private func waitForResultStream(
        ...
        try await withThrowingTaskGroup(of: String.self) { group in
            ...
            do {
                ...
                group.cancelAll()
                logger.error("Apple Speech result wait failed: \(AppLogger.errorMetadata(error), privacy: .public).")
                throw error
            }
        }
    }
