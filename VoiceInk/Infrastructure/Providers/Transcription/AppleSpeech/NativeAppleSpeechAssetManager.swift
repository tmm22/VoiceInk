...
enum NativeAppleSpeechAssetManager {
    private static let logger = Logger(
        subsystem: AppLogger.subsystem, category: "NativeAppleSpeechAssetManager")
    ...
    static func installAsset(for localeIdentifier: String) async -> NativeAppleSpeechAssetState {
        guard #available(macOS 26, *) else {
            ...
            do {
                ...
                logger.error(
                    "Apple Speech asset download failed for '\(localeIdentifier, privacy: .public)': \(AppLogger.errorMetadata(error), privacy: .public)."
                )
                return .failed(error.localizedDescription)
            }
        #else
    ...
    static func languageDisplayName(for localeIdentifier: String) -> String {
        ...
        static func reserveLocaleIfNeeded(for context: AssetContext) async -> Bool {
            ...
            do {
                ...
                logger.warning(
                    "Apple Speech asset reservation failed for '\(context.localeIdentifier, privacy: .public)': \(AppLogger.errorMetadata(error), privacy: .public). Continuing with the installed language assets."
                )
                return true
            }
        }
