                        ...
                        Divider()
                            ...
                            if isAIEnhancementEnabled {
                                VStack(alignment: .leading, spacing: 12) {
                                    ...
                                    if aiService.availableProviders.isEmpty {
                                        ...
                                        Picker("", selection: providerBinding) {
                                            ForEach(aiService.availableProviders) { provider in
                                ...
                                VStack(alignment: .leading, spacing: 12) {
                                    ...
                                    if models.isEmpty {
                                        Text("No models available for \(aiService.selectedProvider.rawValue)")
                                            .foregroundColor(.secondary)
                                            .italic()
                                    } else {
                                        let modelBinding = Binding<String>(
                                            get: { 
                                                if let model = selectedAIModel, models.contains(model) {
                                                    return model
                                                }
                                ...
                                case .builtIn:
                                    Label("Runs locally with bundled formatter.", systemImage: "lock.shield")
                                        .font(.caption)
                                        .foregroundColor(.secondary)
                                case .ollama:
                                    HStack(spacing: 10) {
                                        Label(aiService.isReady ? "Ollama connected" : "Start Ollama to enable enhancement", systemImage: aiService.isReady ? "dot.radiowaves.left.and.right" : "exclamationmark.triangle")
                                            .font(.caption)
                                            .foregroundColor(aiService.isReady ? .secondary : .orange)
