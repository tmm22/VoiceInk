...
- The attached DMG is built from the repository's automated `VoiceInkRelease` packaging flow with coverage disabled, stripping, English-only Pocket TTS dictionary pruning, `UDBZ` compression, and arm64-thinning to keep downloads smaller without removing shipped functionality.
