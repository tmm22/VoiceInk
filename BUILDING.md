...
- `make dev` — build and launch the Debug `VoiceInk.app`
...
Select the `VoiceInk` scheme. Run builds the Debug `VoiceInk.app`; Archive uses Release. `LOCAL_BUILD` applies only through `make local`.
